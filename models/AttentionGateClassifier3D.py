...
from .utils import Depthwise_Conv3D, CBLayer3D,\
    Residual_ConvBlock3D, Residual_Encoder3D, Attention_Gate3D, initialize_weights


class Residual_Attention_Classifier3D(nn.Module):

    def __init__(self, initializer="kaiming"):
        super().__init__()

        # Feature extraction modules
        self.encoder1 = Residual_Encoder3D(1,8)
        self.encoder2 = Residual_Encoder3D(8,16)
        self.encoder3 = Residual_Encoder3D(16,32)
        self.encoder4 = Residual_Encoder3D(32,64)
        self.encoder5 = Residual_ConvBlock3D(64)

        # Attention gates for level 3 and 4
        self.Attn_gate3 = Attention_Gate3D(32,64,32,
                                           ds_rate=4)
        self.Attn_gate4 = Attention_Gate3D(64,64,64,
                                           ds_rate=2)
        ...
        if initializer:
            for m in self.modules():
                if isinstance(m, nn.Conv2d) or isinstance(m, nn.BatchNorm3d):
    ...
    def forward(self, x, mode='Combined'):
        ...
        bsize,_,w,h,d = gating.size()

        out3,grid3 = self.Attn_gate3(x3,gating)
        out4,grid4 = self.Attn_gate4(x4,gating)

        out3 = torch.sum(out3, dim=(2,3,4))/torch.sum(grid3, dim=(2,3,4))
        out4 = torch.sum(out4, dim=(2,3,4))/torch.sum(grid4, dim=(2,3,4))
        out5 = F.avg_pool3d(gating, kernel_size=(w,h,d)).view(bsize,-1)
